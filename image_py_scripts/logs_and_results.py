...
# ROOT = 'C:/Users/Talha/OneDrive - Higher Education Commission/Documents/GitHub/ConvMC/convMC_Data'
# ROOT = 'C:/Users/HP/Git/ConvMC-Net/convMC_Data'
...
def get_modularized_record(project_name, q, db, new_entry, hyper_param_net, params_net, ROOT, SESSION, current_epoch = None):
    ...
    if current_epoch == None:
        log_path = (f'{dir}/{project_name}_Layers_{params_net["layers"]}_TrainInstances_{hyper_param_net["TrainInstances"]}_Epochs_{hyper_param_net["Epochs"]}_lr_{hyper_param_net["Lr"]}.txt')
        return log_path
    elif new_entry == 'Plots':
        model_path = (f'{dir}/{project_name}_Layers_{params_net["layers"]}_TrainInstances_{hyper_param_net["TrainInstances"]}_Epochs_[{current_epoch}_out_of_{hyper_param_net["Epochs"]}]_lr_{hyper_param_net["Lr"]}.png')
        return model_path
    else:
        model_path = (f'{dir}/_Layers_{params_net["layers"]}_TrainInstances_{hyper_param_net["TrainInstances"]}_Epochs_[{current_epoch}_out_of_{hyper_param_net["Epochs"]}]_lr_{hyper_param_net["Lr"]}.pth')
        # model_path = (f'Layers_{params_net["layers"]}_TrainInstances_{hyper_param_net["TrainInstances"]}_Epochs_[{current_epoch}_out_of_{hyper_param_net["Epochs"]}]_lr_{hyper_param_net["Lr"]}.pth')
        return model_path
